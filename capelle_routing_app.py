# app.py
# Capelle route-optimalisatie (alleen wegennet, Google-geocodering)
# Ondersteunt:
...
def main() -> None:
    """
    Run the Streamlit app for drive-only route optimization in Capelle.
    ...
    if not simple_mode:
        st.markdown(
            'Voer een adres per regel in binnen de regio Capelle.\n\n'
    ...
    if st.button('Optimaliseer route'):
        ...
        with timeblock('Total optimization run', logs):
            ...
            with timeblock('Computing network bounding box (WGS84)', logs):
                ...
                # Shrink bbox around the network center to focus on Capelle
