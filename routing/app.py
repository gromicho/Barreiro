...
def _build_distance_matrix_df_km(dist_matrix_m: list[list[float]], addresses: list[str]) -> pd.DataFrame:
    """
    Build a DataFrame to display a distance matrix in Streamlit.

    Requirements:
    - Row labels: summarized addresses.
    - Column labels: indices (same order as rows), 1..n.
    - Values: kilometers, rounded to 1 decimal.

    Args:
        dist_matrix_m: Distance matrix in meters.
        addresses: Addresses in same order as matrix rows.

    Returns:
        DataFrame (float km) rounded to 1 decimal.
    """
    n = len(dist_matrix_m)
    row_labels = [_summarize_address_label(a) for a in addresses]
    col_labels = [str(i) for i in range(1, n + 1)]

    km = (np.array(dist_matrix_m, dtype=float) / 1000.0).round(1)
...
def run_routing_app(*, cfg: RoutingAppConfig) -> None:
    ...
    with timeblock("Total optimization run", logs):
        ...
        # Full UI: show snap distances per address
        if not simple_mode:
            st.subheader("Snapping distances (to road network)")
            for addr, d_m in zip(addresses, snapped_distances_m):
                st.write(f"- {_summarize_address_label(addr)} — {d_m / 1000.0:.2f} km")
        ...
        # Symmetrize and normalize matrix (meters)
        c = np.array(dist_matrix_raw, dtype=float)
        c = 0.5 * (c + c.T)
        np.fill_diagonal(c, 0.0)
        dist_matrix = c.tolist()

        # Full UI: show distance matrix (km) with requested labels/format
        if not simple_mode:
            with st.expander("Afstandsmatrix (km)", expanded=False):
                ...
                try:
                    st.dataframe(
                        ...
                        column_config={
                            col: st.column_config.NumberColumn(col, format="%.1f km")
