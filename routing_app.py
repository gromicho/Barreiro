"""
Shared routing app entry point (Streamlit).

This module contains all common routing UI and logic.
Instance-specific wrappers provide only configuration (store file, drive prefix,
title parameters).
"""
...
class RoutingAppConfig:
    """Configuration for a single routing app instance."""
...
def _cached_drive_graph(data_dir_str: str, drive_prefix: str) -> tuple[object, object]:
    """Streamlit-cached wrapper around load_drive_graph (keyed by args)."""
...
def run_routing_app(*, cfg: RoutingAppConfig) -> None:
    """Run the shared Streamlit routing app for a given configuration."""
    _setup_logging(logfile=cfg.logfile)
    logging.info('run_routing_app() called')
    ...
    # Keep your current default text (you can later parameterize per instance if desired).
    default_text = (
